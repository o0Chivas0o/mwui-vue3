<template>
  <button class="mw-switch" :class="{[`mw-checked`]:value}" @click="toggle">
    <span class="mw-switch-span"></span>
  </button>
</template>
...
.mw-switch {
  ...
  &.mw-checked { background: #1890ff;
    > .mw-switch-span { left: calc(100% - #{$h2} - 2px); }
  }
  &:focus { outline: none; }
  &:active {
    > .mw-switch-span { width: $h2 + 4px; }
  }
  &.mw-checked:active {
    > .mw-switch-span { width: $h2 + 4px; margin-left: -4px; }
  }
}
</style>
